<template>
  <div classs="message">
    ...
    <div v-if="isSelected2==true">
      <div v-for="(item,index) in leaveWordsList" :key="index" @touchstart="touchStart" @touchmove="touchMove($event,index)">
        <div class="showMessageTitleTime" :class="{displayMode:item.read==0}" :data-type="item.type">
          <span @click="viewMessageDetails(item)" class="line-limit-length">{{item.replyContent}}</span>
          <span class="date">{{item.replyTime}}</span>
        </div>
        <div class="delete" @click="deleteMessage(item.id)">删除</div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      async getLeaveWordsList(){
        let url = '/serviceLeaveWords/getLeaveWordsList';
        let data = {partyId:this.$store.state.wxUserInfo.partyId}
        let result = await request.demand(url,data,'GET');
        this.leaveWordsList = result.data;
        console.log(this.leaveWordsList);
      },
      ...
      viewMessageDetails(item){
        var url="";
        var leaveWordsDetails = JSON.stringify(item);
        url = "/pages/"
        if(item.messageType == 1||item.messageType == 2)
          url="/pages/message_details/main?messageDetails="+ messageDetails;
        // if(item.type==2)
